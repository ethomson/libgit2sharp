...
namespace LibGit2Sharp.Tests
{
    public class RepositoryFixture : BaseFixture
    {
        ...
        public void CanCloneBarely(string url)
        {
            ...
            using (Repository repo = Repository.Clone(url, scd.RootedDirectoryPath, opts))
            {
                string dir = repo.Info.Path;
                Assert.True(Path.IsPathRooted(dir));
                Assert.True(Directory.Exists(dir));
        ...
        public void WontCheckoutIfAskedNotTo()
        {
            ...
            using (Repository repo = Repository.Clone(TestRepoUrl, scd.RootedDirectoryPath, opts))
            {
